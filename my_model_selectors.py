...
class SelectorBIC(ModelSelector):
    ...
    def select(self):
        ...
        for n_comp in range(self.min_n_components,self.max_n_components+1):
            ...
            try:
                hmm_model, score = self.train_model(n_comp)
            except:
                pass
            # Calculate BIC score
            parameters = n_comp **2 + 2 * n_comp * hmm_model.n_features - 1
...
class SelectorCV(ModelSelector):
    ...
    def train_model(self, n_states, X_train, length_train, X_test, length_test):
        warnings.filterwarnings("ignore", category=DeprecationWarning)
        try:
            hmm_model = GaussianHMM(n_components=n_states, covariance_type="diag", n_iter=1000,
    ...
    def select(self):
        ...
        for n_comp in range(self.min_n_components,self.max_n_components+1):
            ...
            for cv_train_idx, cv_test_idx in KFold(n_splits).split(self.sequences):
                ...
                # model is the HMM model created for n_components
                try:
                    hmm_model, test_score = self.train_model(n_comp, X_train, length_train, X_test, length_test)
                except:
                    pass
            ...
            selected_model, score = self.train_model(best_n_comp, self.X, self.lengths, self.X, self.lengths)   
        # TODO implement model selection based on BIC scores
        return selected_model